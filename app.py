...
Interactive UI for dataset creation, model training, evaluation, and prediction.
...
7.  Binary classifier uses softmax+argmax throughout (fixed in train_model.py too).
...
def model_exists():
    return (MODELS_DIR / "best_model.keras").exists()
...
with st.sidebar:
    ...
    if model_exists():
        ...
        st.markdown('<div style="color:#f59e0b;font-family:Space Mono,monospace;font-size:.75rem;">⚠ No trained model</div>', unsafe_allow_html=True)
...
→ Softmax (N classes)""", language="text")
        ...
        if state == "error":
            ...
            if proc_alive:
                with st.spinner("🔧 Initializing model — waiting for first epoch…"):
                    time.sleep(2)
                st.experimental_rerun()
            else:
                # Process died before writing anything — check status for error
                st.error("❌ Training process exited unexpectedly before starting. Check train_model.py for errors.")
                st.session_state["train_pid"] = None
...
with tab3:
    ...
    eval_data = read_eval()
    if not eval_data:
        st.info("No evaluation results yet. Train your model first.")
    else:
